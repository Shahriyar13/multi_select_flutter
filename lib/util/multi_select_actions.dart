...
class MultiSelectActions<T> {
  ...
  List<MultiSelectItem<T>> updateSearchQuery(
    ...
    if (val != null && val.trim().isNotEmpty) {
      ...
      for (var item in allItems) {
        if ((item.value.toString()).toLowerCase().contains(val.toLowerCase())) {
  ...
  List<MultiSelectItem<T>> separateSelected(List<MultiSelectItem<T>> list) {
    ...
    _nonSelectedItems.sort((a, b) => (a.value.toString()).compareTo(b.value.toString()));
    _selectedItems.addAll(list.where((element) => element.selected));
    _selectedItems.sort((a, b) => (a.value.toString()).compareTo(b.value.toString()));
